...
for(KBAform in KBAforms){
  ...
  # Load KBA Canada Proposal Form
        # Visible sheets
  home <- read.xlsx(KBAform, sheet = "HOME")
  proposer <- read.xlsx(KBAform, sheet = "1. PROPOSER")
  site <- read.xlsx(KBAform, sheet = "2. SITE")
  species <- read.xlsx(KBAform, sheet = "3. SPECIES")
  ecosystems <- read.xlsx(KBAform, sheet = "4. ECOSYSTEMS & C")
  threats <- read.xlsx(KBAform, sheet = "5. THREATS")
  review <- read.xlsx(KBAform, sheet = "6. REVIEW")
  citations <- read.xlsx(KBAform, sheet = "7. CITATIONS")
  check <- read.xlsx(KBAform, sheet = "8. CHECK")
  
        # Invisible sheets
  checkboxes <- read.xlsx(KBAform, sheet = "checkboxes")
  resultsSpecies <- read.xlsx(KBAform, sheet = "results_species")
  resultsEcosystems <- read.xlsx(KBAform, sheet = "results_ecosystems")
  ...
  if(batch){
    doc <- paste0(output, "/", "Summary_", str_replace_all(string=nationalName, pattern=c(":| |\\(|\\)"), repl=""), "_", Sys.Date(), ".docx")
  }else{
    doc <- docs[loop]
  }
  
  # Save
  renderInlineCode(template, doc)
  Sys.sleep(10)
